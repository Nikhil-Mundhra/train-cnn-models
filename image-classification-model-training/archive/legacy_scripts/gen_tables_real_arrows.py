def gen_table(top, bot, is_l3=False):
    ...
    for i, name in enumerate(top):
        ...
        if i < len(top) - 1:
            html.append(f'      <td style="padding: 0 10px; color: currentColor; font-size: 24px; font-weight: bold; text-align: center;">→</td>')
    html.append('    </tr>')
    ...
    html.append(f'      <td style="color: currentColor; font-size: 24px; font-weight: bold; text-align: center; padding: 5px 0;">↓</td>')
    html.append('    </tr>')
...
with open('tables_real_arrows.txt', 'w') as f:
    f.write("=== L1 ===\n")
