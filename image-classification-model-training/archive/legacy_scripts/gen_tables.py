def gen_table(top, bot, is_l3=False):
    ...
    for i, name in enumerate(top):
        html.append(f'      <td style="width: 120px;"><div style="padding: 8px 0; border: 2px solid #3B82F6; border-radius: 8px; background: #2a2b2e; color: #fff; font-weight: bold; text-align: center;">{name}</div></td>')
        if i < len(top) - 1:
            html.append(f'      <td style="padding: 0 10px; color: #3B82F6; font-size: 18px; text-align: center;">▶</td>')
    html.append('    </tr>')
    ...
    html.append(f'      <td style="color: #3B82F6; font-size: 22px; text-align: center; padding: 5px 0;">▼</td>')
    html.append('    </tr>')
