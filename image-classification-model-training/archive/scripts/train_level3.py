...
Level 3 Specialist Training — Stratified 5-Fold Cross-Validation.

Model:       ResNet-50 (IMAGENET1K_V2 weights)
Task:        Specialist (e.g. Macular, Diabetic, Vascular, Fluid, Structural)
...
from models.level3_specialist import build_specialist
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Train the Level 3 OCT Specialist",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    # Paths
    p.add_argument("--specialist",     required=True,
                   choices=["Macular", "Diabetic", "Vascular", "Fluid", "Structural"],
                   help="Which specialist branch to train")
    ...
    p.add_argument("--resume",         action="store_true",
                   help="Auto-detect latest checkpoint and resume mid-fold training")
...
def setup_logging(log_dir: str, mode: str, smoke_test: bool) -> None:
    """Configure root logger to write to stdout and a log file."""
    log_path = Path(log_dir) / mode
...
def main() -> None:
    ...
    logger.info("")
    logger.info("=" * 60)
    logger.info("  OCT Specialist — Level 3 Training (%s)", args.specialist)
    if args.smoke_test:
    ...
    MODE = f"level3_{args.specialist.lower()}"
    
    setup_logging(args.log_dir, MODE, args.smoke_test)
    ...
    resume_path = None
    if args.resume:
        ckpt_dir = Path(args.checkpoint_dir) / MODE
        if ckpt_dir.exists():
            resume_files = sorted(ckpt_dir.glob("fold*_resume.pth"))
            if resume_files:
                resume_path = str(resume_files[-1])
                resume_fold = int(Path(resume_path).stem.replace("fold", "").replace("_resume", ""))
                logger.info("Auto-resume detected: Fold %d from %s", resume_fold + 1, resume_path)
            else:
                logger.info("No resume checkpoints found. Starting fresh.")
    
    # ── K-Fold Cross-Validation Loop ──────────────────────────────────────────
    all_fold_metrics = []

    for fold_id, (train_loader, val_loader) in enumerate(fold_loaders):
        if fold_id < resume_fold:
            logger.info("Skipping FOLD %d (already completed or resuming from later fold)", fold_id + 1)
            continue
        ...
        fold_metrics = trainer.train(
            ...
            fold_id=fold_id,
            resume_checkpoint_path=resume_path if fold_id == resume_fold else None,
        )
