...
class HierarchicalUNetGradCAM:
    """
    Grad-CAM for Hierarchical U-Net models (unified segmentation/classification).
    Supports targeting a specific head (1, 2, or 3) and a specific class within that head.
    """
    def __init__(self, model, target_layer):
        self.model = model
    ...
    def generate_cam(self, input_tensor, target_head=2, sub_head=None, target_class=None):
        ...
        Args:
            input_tensor (torch.Tensor): Preprocessed image tensor (1, 1, H, W)
            target_head (int): Which head to explain (1, 2, or 3). Defaults to 2 (Pathology Routing).
            sub_head (str, optional): The severity family (e.g., 'macular') if target_head == 3.
            target_class (int, optional): The class within the target head. 
                                          If None, uses the class with the highest score.
        ...
        # Forward pass returning just classification logits
        outputs = self.model(input_tensor, task="classification")
    ...
    def overlay_cam(img_pil: Image.Image, cam: np.ndarray, alpha: float = 0.5) -> Image.Image:
        ...
        if len(img_arr.shape) == 2:
            # It's a grayscale image (H, W)
            img_cv2 = cv2.cvtColor(img_arr, cv2.COLOR_GRAY2BGR)
        else:
            # It's an RGB image (H, W, 3)
            img_cv2 = cv2.cvtColor(img_arr, cv2.COLOR_RGB2BGR)
