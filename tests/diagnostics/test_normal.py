...
normal_rows = manifest[manifest["image_path"].str.contains("NORMAL") | manifest["image_path"].str.contains("Normal")]

print(f"Testing on {min(5, len(normal_rows))} Normal images:")
for i in range(min(5, len(normal_rows))):
    row = normal_rows.iloc[i]
